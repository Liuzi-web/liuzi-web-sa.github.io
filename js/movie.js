(function($) {
	...
	function init(cus_setting) {
		...
		var target = this;
		var sliderframestring =
			'<div class =\'slide-wrap\'><div class=\'border\'><div class=\'slide\'><div class=\'shift right\'></div><i class=\'shift right\'></i><div class=\'shift left\'></div><i class =\'shift left\'></i><ul><li></li><li></li></ul></div></div></div>';
		var sliderhtml = $(sliderframestring);
		sliderhtml.appendTo(target);

		$.each(init_setting.JSON[init_setting.subject], function(i, field) {
			target.find('ul>li:first').after('<li><img src=' + field.imgpath + '></li>');
		});

		target.width("700px");
		target.find('.slide-wrap').height("400px");
		target.find('ul').css({
			left: -covered
		});
		target.find('li').css("margin-right", init_setting.margin);
		target.find('li').width("300px");
		target.find('li').height("auot");
		target.find('img').width("300px");
